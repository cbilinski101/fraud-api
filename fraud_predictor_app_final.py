...
if "slide_index" not in st.session_state:
    ...
    st.session_state.animate_once = False

col1, col2, col3 = st.columns([1, 5, 1])
with col2:
    img_path = os.path.join(slide_dir, image_files[st.session_state.slide_index])
    st.image(Image.open(img_path), caption=f"{image_files[st.session_state.slide_index]}", use_container_width=True)
...
if st.session_state.animate_once:
    time.sleep(3)
